        ...
        if dice==7:
            ...
            for tile in tiles.values():
                if tile['number'] == dice:
                    if tile['thief'] == True:
                        continue
                    for point_coordinates in Board.get_point_coordinates_around_tile(tile['position'], Board.get_board_half(row, actual_board_size=self.board.size+1)): # TODO complete 'row' and 'size'
